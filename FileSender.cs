...
namespace SeaWarsGame
{
    public static class FileSender
    {
        public class UdpFileServer
        {
            ...
            [STAThread]
            public static void Send(String SremoteIPAddress, String fileName)
            {
                try
                {
                    // Получаем удаленный IP-адрес и создаем IPEndPoint
                    // Console.WriteLine("Введите удаленный IP-адрес");
                    remoteIPAddress = IPAddress.Parse(SremoteIPAddress.ToString());//"127.0.0.1");
                    ...
                    fs = new FileStream(fileName.ToString(), FileMode.Open, FileAccess.Read);
